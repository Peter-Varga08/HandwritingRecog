...
def resize_pad(img, height_all, width_all, padding_number=0):
    ...
    if height > width:
        ...
        if left_pad + right_pad + new_width != 40:
            right_pad += 1
        resized_pad_img = cv2.copyMakeBorder(resized_img, 0, 0, left_pad, right_pad, cv2.BORDER_CONSTANT,
                                             value=padding_number)
    else:
        resize_factor = height_all / height
        new_height = int(img.shape[0] * resize_factor)
        dim = (width_all, new_height)
        ...
        if top_pad + bottom_pad + new_height != 40:
            bottom_pad += 1
        resized_pad_img = cv2.copyMakeBorder(resized_img, top_pad, bottom_pad, 0, 0, cv2.BORDER_CONSTANT,
                                             value=padding_number)
