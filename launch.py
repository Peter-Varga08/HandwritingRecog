...
from Text_Segmentation.characterSegmentation import (
    character_segmentation, clean_image, destructure_characters,
    select_slides, slide_over_word)
from Text_Segmentation.lineSegmentation import line_segmentation
from Text_Segmentation.segmentation_to_recog import (TheRecognizer,
                                                     get_label_probability)
from Text_Segmentation.wordSegmentation import word_segmentation
from font2image import labeltotext, styletotext

warnings.filterwarnings("ignore")


def run_pipeline(img_name: str, img_path: str, results_path: str = "results/"):
    aStar_path = f"paths/{img_path[0:-4]}"
    ...
    (
        characters_word_line,
        single_character_widths,
        mean_character_width,
    ) = character_segmentation(words_in_lines)

    model = TheRecognizer()
    model.load_model(
        model.load_checkpoint("40_char_rec.ckpt", map_location=torch.device("cpu"))
    )
    name2idx = {
        "Alef": 0,
        "Ayin": 1,
                ...
                if character_segment.shape[1] > mean_character_width + np.std(
                    ...
                    try:
                        ...
                        predicted_char_num = round(
                            character_segment.shape[1] / mean_character_width
                        )
                        ...
                        for label in predicted_labels:
                            predictions_string = (
                                f"{predictions_string}, {list(name2idx.keys())[label]}"
                            )
                    ...
                    if character_segment.size != 0:
                        ...
                        predicted_label, probability = get_label_probability(
                            character_segment, model
                        )
    ...
    dominant_style = get_style_SVM(
        all_segmented_characters, all_segmented_labels, all_segmented_labels
    )
...
# TODO
def run_demo_pipeline(img_path: str, results_path: str = "results/"):
    pass
...
def main() -> None:
    for img_name in tqdm(test_images):
        ...
        try:
            ...
            run_pipeline(img_name=img_name, img_path=img_path)
            break
